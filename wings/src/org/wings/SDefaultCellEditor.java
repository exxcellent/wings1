    ...
    implements STableCellEditor
{
     ...
     *
     */
    private final SRequestListener eventChecker;
    ...
    // default init
    {
        ...
        eventChecker = new FireDelayedEventsRequestListener(this);
    ...
    private static class FireDelayedEventsRequestListener implements SRequestListener {

        private final WeakReference ownerRef;

        public FireDelayedEventsRequestListener(SDefaultCellEditor owner) {
            ownerRef = new WeakReference(owner);
        }

        public void processRequest(SRequestEvent e) {
            if ( e.getType()==SRequestEvent.DISPATCH_DONE ) {
                SDefaultCellEditor owner = (SDefaultCellEditor) ownerRef.get();
                if (owner != null)
                    owner.checkFireEvents();
                else
