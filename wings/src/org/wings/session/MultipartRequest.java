        ...
        try {
            ...
            while(currentByte != -1) {
                ...
                while ((currentByte = mimeStream.read()) != -1 && !done) {
                    header.write(currentByte); // okay -- let us asume no special characters in the header
                ...
                headers = parseHeader(header.toString(req.getCharacterEncoding()));
                header.reset();     // Clear buffer for upcoming parameter
