...
final class SessionServlet
    ...
    public final synchronized void doGet(HttpServletRequest req,
        ...
        try {
            ...
            logger.info("Socket exception (guess client aborted, hence harmless)", ioException);
            handleException(req, response, ioException);
        } catch (Throwable e) {
            logger.warn("Unexpected exception during delivery phase: ", e);
    ...
    protected void handleException(HttpServletRequest req,
        ...
        try {
            if (getSession().getDispatchErrorHandler() != null) {
                // Tell app that an ex occured
                getSession().getDispatchErrorHandler().handleException(e);
                // reload page (render error page)
                String redirectAddress = req.getRequestURL().toString();
                if (getSession().getRedirectAddress() != null) {
                    redirectAddress = getSession().getRedirectAddress();
                    getSession().setRedirectAddress(null);
                }
                res.sendRedirect(redirectAddress);
            } else {
            if (errorFrame == null) {
            ...
            logger.info("IOException occurred during errorHandling. Guess this is a harmless aborted request.");
        } catch(IllegalStateException illegalState) {
            logger.info("IllegalStateException occurred during errorHandling. Guess this is due to an closed response.");
        } catch (Throwable ex) {
            logger.warn(SessionServlet.class.getName() + " handleException", ex);
        }
    }
